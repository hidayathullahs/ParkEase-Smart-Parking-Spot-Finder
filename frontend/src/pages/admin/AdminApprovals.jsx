...
const AdminApprovals = () => {
    ...
    const fetchPending = async () => {
        setLoading(true);
        try {
            const res = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}/admin/parkings/pending', {
...
const handleReject = async () => {
    ...
    try {
        await axios.put(`http://localhost:5000/api/admin/parkings/${rejectId}/reject`, { reason: rejectReason }, {
            headers: { Authorization: `Bearer ${token}` }
        });
        // Remove from list
